...
const unresolved_error_code: i32 = std.math.minInt(i32);
...
pub const Analyzer = struct {
    ...
    fn collectTopLevelSymbols(self: *Analyzer, program: ast.Program) errors.SemanticError!void {
        ...
        try self.resolveErrorCodeValues(program);
    ...
    fn resolveErrorCodeValues(self: *Analyzer, program: ast.Program) errors.SemanticError!void {
        var it = self.error_codes.iterator();
        while (it.next()) |entry| {
            if (!std.mem.eql(u8, entry.key_ptr.*, "ErrorGuard")) {
                entry.value_ptr.* = unresolved_error_code;
            }
        }

        for (program.functions.items) |node| {
            if (node.data != .error_decl) continue;
            const decl = node.data.error_decl;
            if (decl.code_kind == .explicit) {
                if (self.error_codes.getPtr(decl.name)) |ptr| {
                    ptr.* = decl.explicit_code;
                }
            }
        }
        ...
        while (progress) {
            ...
            for (program.functions.items) |node| {
                if (node.data != .error_decl) continue;
                const decl = node.data.error_decl;
                if (decl.code_kind != .alias) continue;

                const current_code = self.error_codes.get(decl.name) orelse continue;
                if (current_code != unresolved_error_code) continue;

                const alias_name = decl.alias_name orelse continue;
                if (self.error_codes.get(alias_name)) |alias_code| {
                    if (alias_code != unresolved_error_code) {
                        if (self.error_codes.getPtr(decl.name)) |ptr| {
                            ptr.* = alias_code;
                            progress = true;
                        }
        ...
        for (program.functions.items) |node| {
            ...
            if (decl.code_kind != .alias) continue;
            const current_code = self.error_codes.get(decl.name) orelse continue;
            if (current_code == unresolved_error_code) {
                if (decl.alias_name) |alias_name| {
                    self.reportNodeErrorFmt(node, "Unresolved error alias '{s}'", .{alias_name}, "Alias must reference a declared error with a resolved code");
                } else {
                    self.reportNodeError(node, "Unresolved error alias", "Alias must reference a declared error with a resolved code");
                }
            }
        }

        var used_codes = std.AutoHashMap(i32, void).init(self.allocator);
        defer used_codes.deinit();
        var codes_it = self.error_codes.iterator();
        while (codes_it.next()) |entry| {
            const code = entry.value_ptr.*;
            if (code != unresolved_error_code) {
                try used_codes.put(code, {});
            }
        }

        var next_auto_code: i32 = 1;
        for (program.functions.items) |node| {
            if (node.data != .error_decl) continue;
            const decl = node.data.error_decl;
            if (decl.code_kind != .auto) continue;

            const current_code = self.error_codes.get(decl.name) orelse continue;
            if (current_code != unresolved_error_code) continue;

            while (used_codes.contains(next_auto_code)) {
                next_auto_code += 1;
            }
            if (self.error_codes.getPtr(decl.name)) |ptr| {
                ptr.* = next_auto_code;
            }
            try used_codes.put(next_auto_code, {});
            next_auto_code += 1;
        }
    }

    fn errorCodeMatchesName(self: *Analyzer, code: i32, error_name: []const u8) bool {
        if (self.error_codes.get(error_name)) |resolved| {
            return resolved != unresolved_error_code and resolved == code;
        }
        return false;
    }

    fn handlerMatchesErrorName(self: *Analyzer, handler: ast.ErrorHandler, error_name: []const u8) bool {
        if (handler.error_name) |handler_name| {
            return std.mem.eql(u8, handler_name, error_name);
        }
        if (handler.error_code) |handler_code| {
            return self.errorCodeMatchesName(handler_code, error_name);
        }
        return true;
    }
    ...
    fn collectLabelsFromNode(self: *Analyzer, node: *ast.Node, labels: *std.StringHashMap(void)) errors.SemanticError!void {
        switch (node.data) {
            ...
            .defer_stmt => |defer_stmt| {
                try self.collectLabelsFromNode(defer_stmt.expression, labels);
            },
            else => {},
        }
    }
    ...
    fn collectFunctionFlows(self: *Analyzer, statements: []const *ast.Node, send_list: *std.ArrayList([]const u8), solicit_list: *std.ArrayList([]const u8)) errors.SemanticError!void {
        for (statements) |stmt| {
            switch (stmt.data) {
                ...
                .defer_stmt => |defer_stmt| {
                    try self.collectFunctionFlows(&[_]*ast.Node{defer_stmt.expression}, send_list, solicit_list);
                },
                else => {},
            }
        }
    ...
    fn collectDeclaredNamesFromStatements(self: *Analyzer, statements: []const *ast.Node, names: *std.StringHashMap(void)) errors.SemanticError!void {
        for (statements) |stmt| {
            switch (stmt.data) {
                ...
                .defer_stmt => |defer_stmt| {
                    try self.collectDeclaredNamesFromStatements(&[_]*ast.Node{defer_stmt.expression}, names);
                },
                else => {},
            }
        }
        ...
        switch (stmt.data) {
            ...
            .defer_stmt => |defer_stmt| {
                if (self.loop_depth > 0) {
                    self.reportNodeError(stmt, "'defer' is not supported inside loops", "Move defer outside loop body");
                }
                try self.analyzeExpression(defer_stmt.expression);
            },
            .if_stmt => |if_stmt| {
    ...
    fn analyzeHandledCall(self: *Analyzer, node: *ast.Node, handled: ast.HandledCallStmt, labels: *const std.StringHashMap(void)) errors.SemanticError!void {
        ...
        for (handled.handlers.items) |handler| {
            ...
            } else if (handler.error_code == null) {
                switch (handler.kind) {
        ...
        if (self.function_flows.get(call_name)) |flow| {
            for (handled.handlers.items) |handler| {
                ...
                if (handler.error_name) |error_name| {
                ...
                } else if (handler.error_code) |error_code| {
                    var matched = false;
                    for (relevant_list) |sent_name| {
                        if (self.errorCodeMatchesName(error_code, sent_name)) {
                            matched = true;
                            break;
                        }
                    }
                    if (!matched) {
                        const hint = switch (handler.kind) {
                            .send => "Called function never sends this error code",
                            .solicit => "Called function never solicits this error code",
                        };
                        self.reportNodeWarningFmt(node, "Handler for error code {d} is ignored", .{error_code}, hint);
                    }
                }
            }
            ...
            if (!has_send_catch_all) {
                for (flow.send_errors.items) |sent_name| {
                    ...
                    for (handled.handlers.items) |handler| {
                        ...
                        if (self.handlerMatchesErrorName(handler, sent_name)) {
                            handled_error = true;
                            break;
                        }
                    }
            ...
            if (!has_solicit_catch_all) {
                for (flow.solicit_errors.items) |solicit_name| {
                    ...
                    for (handled.handlers.items) |handler| {
                        ...
                        if (self.handlerMatchesErrorName(handler, solicit_name)) {
