...
pub const CodeGenerator = struct {
    ...
    fn currentDeferMarker(self: *CodeGenerator) usize {
    ...
    fn generateSolicitCallback(self: *CodeGenerator, callback_id: usize, handled: ast.HandledCallStmt, caller_bindings: []const SolicitCaptureBinding, callee_bindings: []const SolicitCaptureBinding) errors.CodegenError!c.LLVMValueRef {
        ...
        for (handled.handlers.items) |handler| {
            ...
            const cond = if (handler.error_name) |error_name| blk: {
                ...
                const code_val = c.LLVMConstInt(i32_ty, code_bits, 1);
                break :blk c.LLVMBuildICmp(self.builder, c.LLVMIntEQ, err_code_param, code_val, "solicit.match.code");
            } else blk: {
    ...
    fn generateHandledCall(self: *CodeGenerator, handled: ast.HandledCallStmt, expected_type: ?[]const u8) errors.CodegenError!c.LLVMValueRef {
        ...
        for (handled.handlers.items) |handler| {
            ...
            const error_cond = if (handler.error_name) |error_name| blk: {
                ...
                const code_bits: c_ulonglong = @bitCast(@as(i64, error_code));
                const code_val = c.LLVMConstInt(i32_ty, code_bits, 1);
                break :blk c.LLVMBuildICmp(self.builder, c.LLVMIntEQ, err_now, code_val, "err.match.code");
            } else blk: {
                const zero = c.LLVMConstInt(i32_ty, 0, 0);
            ...
            try CodeGenerator.pushScope(self);
            const handler_marker = self.currentDeferMarker();
            ...
            if (c.LLVMGetBasicBlockTerminator(c.LLVMGetInsertBlock(self.builder)) == null) {
                try self.runDeferredActionsFrom(handler_marker);
            }
            CodeGenerator.popScope(self);
                ...
                if (un.op == '&') {
                    const operand_type = switch (un.operand.data) {
                        .identifier => |ident| blk: {
                            ...
                            break :blk try self.inferType(un.operand);
                        },
                        .qualified_identifier => blk: {
                            const pair = try self.getQualifiedFieldPtrAndType(un.operand);
                            break :blk self.getTypeNameFromLLVMType(pair.ty);
                        },
                        else => try self.inferType(un.operand),
                    };
                    return try std.fmt.allocPrint(self.allocator, "ptr<{s}>", .{operand_type});
                }
