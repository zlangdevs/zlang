...
extern void* zig_create_defer_stmt(void* expression);
...
extern int zlang_get_column(void* scanner);
extern char* zlang_get_text(void* scanner);
...
%token TOKEN_FUN TOKEN_IF TOKEN_ELSE TOKEN_FOR TOKEN_RETURN TOKEN_DEFER TOKEN_VOID TOKEN_BREAK TOKEN_CONTINUE TOKEN_GOTO TOKEN_USE TOKEN_WRAP TOKEN_ENUM TOKEN_STRUCT TOKEN_UNION TOKEN_DOT TOKEN_NULL TOKEN_CONST TOKEN_WHEN TOKEN_ERROR TOKEN_SEND TOKEN_SOLICIT TOKEN_ON
...
defer_statement:
    TOKEN_DEFER expression {
        zlang_set_location(@$.first_line, @$.first_column); $$ = zig_create_defer_stmt($2);
    }
;
...
void yyerror(const char* s) {
    ...
    int col = zlang_get_column(current_scanner);
    const char* token = zlang_get_text(current_scanner);
    if (token != NULL && token[0] != '\0') {
        char token_preview[64];
        size_t out = 0;
        size_t i = 0;
        while (token[i] != '\0' && out < sizeof(token_preview) - 1 && i < 48) {
            unsigned char ch = (unsigned char)token[i];
            token_preview[out++] = (ch >= 32 && ch <= 126) ? (char)ch : ' ';
            i++;
        }
        while (out > 0 && token_preview[out - 1] == ' ') {
            out--;
        }
        token_preview[out] = '\0';
        char message[1024];
        if (out > 0) {
            snprintf(message, sizeof(message), "%s near '%s'", s, token_preview);
            zig_record_parse_error(line, col, message);
        } else {
            zig_record_parse_error(line, col, s);
        }
    } else {
        zig_record_parse_error(line, col, s);
    }
